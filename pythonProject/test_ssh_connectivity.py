...
try:
    from app.utils.hmac_generator import generate_hmac_for_account_transfer, generate_hmac_for_phone_transfer
except ImportError:
    print("❌ No se puede importar HMAC generator. Asegúrate de estar en el directorio correcto.")
    print("Ejecuta desde: c:\\Users\\juanc\\Documents\\GitHub\\redes\\bancoPy\\pythonProject\\")
    sys.exit(1)
    ...
    try:
        result = subprocess.run(
            ['ssh', '-o', f'ConnectTimeout={timeout}', '-o', 'BatchMode=yes', 
             '-o', 'StrictHostKeyChecking=no', f'{host}', 'echo "SSH OK"'],
            capture_output=True, 
            text=True,
            timeout=timeout + 2
        )
        ...
        print("⚠️  SSH no disponible en este sistema. Verificando solo conectividad de red...")
...
def main():
    ...
    for bank_code, bank_config in banks.items():
        ...
        if api_ok:
            ...
            print(f"   Transfer: ⚠️ SKIP (API no disponible)")
            print(f"   Móvil: ⚠️ SKIP (API no disponible)")
    ...
    for bank_code, tests in results.items():
        ...
        ssh_status = "✅" if tests.get('ssh') else ("❌" if tests.get('ssh') is not None else "⚠️")
        ...
        print(f"{bank_name:25} | SSH: {ssh_status} | API: {api_status} | Transfer: {transfer_status} | Móvil: {movil_status}")
    
    # Recomendaciones
    print(f"\n💡 RECOMENDACIONES")
    print("-" * 30)
    
    failed_banks = [code for code, tests in results.items() if not tests.get('api', False)]
    if failed_banks:
        print("• Bancos sin conectividad API:")
        for code in failed_banks:
            print(f"  - {banks[code]['name']} ({banks[code]['url']})")
    
    transfer_failed = [code for code, tests in results.items() if tests.get('api', False) and not tests.get('transfer', False)]
    if transfer_failed:
