...
class BankingGUI:
    ...
    def setup_server_panel(self, parent):
        ...
        ttk.Button(server_frame, text="Iniciar Servidor", command=self.start_server).grid(row=0, column=1, padx=5)
        ttk.Button(server_frame, text="Detener Servidor", command=self.stop_server).grid(row=0, column=2, padx=5)
        ttk.Button(server_frame, text="Verificar Estado", command=self.check_server_status).grid(row=0, column=3, padx=5)
    ...
    def create_user_tab(self):
        ...
        form_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 10), pady=(0, 10))
        ...
        ttk.Label(form_frame, text="Teléfono:").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.user_phone_entry = ttk.Entry(form_frame, width=30)
        self.user_phone_entry.grid(row=2, column=1, pady=2, padx=(5, 0))
        
        ttk.Label(form_frame, text="Contraseña:").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.user_password_entry = ttk.Entry(form_frame, width=30, show="*")
        self.user_password_entry.grid(row=3, column=1, pady=2, padx=(5, 0))
        
        ttk.Button(form_frame, text="Crear Usuario", command=self.create_user).grid(row=4, column=0, columnspan=2, pady=10)
        ...
        self.user_tree = ttk.Treeview(list_frame, columns=("ID", "Nombre", "Email", "Teléfono"), show="headings", height=10)
        ...
        # Botones de acción
        ttk.Button(list_frame, text="Actualizar Lista", command=self.load_users).grid(row=1, column=0, pady=(10, 0))
        ttk.Button(list_frame, text="Eliminar Usuario", command=self.delete_user).grid(row=1, column=1, pady=(10, 0))
    ...
    def create_account_tab(self):
        ...
        ttk.Label(form_frame, text="Número de Cuenta:").grid(row=0, column=0, sticky=tk.W, pady=2)
        ...
        currency_combo = ttk.Combobox(form_frame, textvariable=self.account_currency_var, 
                                    values=["CRC", "USD", "EUR"], width=27, state="readonly")
        currency_combo.grid(row=1, column=1, pady=2, padx=(5, 0))
        
        ttk.Label(form_frame, text="Saldo Inicial:").grid(row=2, column=0, sticky=tk.W, pady=2)
        ...
        ttk.Label(form_frame, text="ID Usuario (opcional):").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.account_user_id_entry = ttk.Entry(form_frame, width=30)
        self.account_user_id_entry.grid(row=3, column=1, pady=2, padx=(5, 0))
        
        ttk.Button(form_frame, text="Crear Cuenta", command=self.create_account).grid(row=4, column=0, columnspan=2, pady=10)
        
        # Lista de cuentas
        list_frame = ttk.LabelFrame(account_frame, text="Lista de Cuentas", padding="10")
        list_frame.grid(row=0, column=1, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        
        self.account_tree = ttk.Treeview(list_frame, columns=("ID", "Número", "Moneda", "Saldo"), show="headings", height=10)
        ...
        # Botones
        ttk.Button(list_frame, text="Actualizar Lista", command=self.load_accounts).grid(row=1, column=0, pady=(10, 0))
        ttk.Button(list_frame, text="Actualizar Saldo", command=self.update_account_balance).grid(row=1, column=1, pady=(10, 0))
    ...
    def create_phone_link_tab(self):
        ...
        form_frame = ttk.LabelFrame(phone_frame, text="Crear Enlace Telefónico", padding="10")
        form_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 10), pady=(0, 10))
        ...
        ttk.Label(form_frame, text="Teléfono:").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.phone_number_entry = ttk.Entry(form_frame, width=30)
        self.phone_number_entry.grid(row=1, column=1, pady=2, padx=(5, 0))
        
        ttk.Button(form_frame, text="Crear Enlace", command=self.create_phone_link).grid(row=2, column=0, columnspan=2, pady=10)
        
        # Lista de enlaces
        list_frame = ttk.LabelFrame(phone_frame, text="Enlaces Existentes", padding="10")
        list_frame.grid(row=0, column=1, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        
        self.phone_tree = ttk.Treeview(list_frame, columns=("ID", "Cuenta", "Teléfono", "Fecha"), show="headings", height=10)
        ...
        # Scrollbar
        phone_scrollbar = ttk.Scrollbar(list_frame, orient=tk.VERTICAL, command=self.phone_tree.yview)
        phone_scrollbar.grid(row=0, column=1, sticky=(tk.N, tk.S))
        self.phone_tree.configure(yscrollcommand=phone_scrollbar.set)
        
        # Botones
        ttk.Button(list_frame, text="Actualizar Lista", command=self.load_phone_links).grid(row=1, column=0, pady=(10, 0))
        ttk.Button(list_frame, text="Eliminar Enlace", command=self.delete_phone_link).grid(row=1, column=1, pady=(10, 0))
    ...
    def create_transfer_tab(self):
        ...
        traditional_frame = ttk.LabelFrame(transfer_frame, text="Transferencia SINPE Tradicional", padding="10")
        traditional_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 10), pady=(0, 10))
        
        ttk.Label(traditional_frame, text="Cuenta Origen:").grid(row=0, column=0, sticky=tk.W, pady=2)
        ...
        ttk.Label(traditional_frame, text="Cuenta Destino:").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.transfer_to_entry = ttk.Entry(traditional_frame, width=30)
        self.transfer_to_entry.grid(row=1, column=1, pady=2, padx=(5, 0))
        
        ttk.Label(traditional_frame, text="Monto:").grid(row=2, column=0, sticky=tk.W, pady=2)
        ...
        ttk.Label(traditional_frame, text="Descripción:").grid(row=3, column=0, sticky=tk.W, pady=2)
        ...
        ttk.Button(traditional_frame, text="Realizar Transferencia", command=self.create_transfer).grid(row=4, column=0, columnspan=2, pady=10)
        
        # Transferencia móvil
        mobile_frame = ttk.LabelFrame(transfer_frame, text="Transferencia SINPE Móvil", padding="10")
        mobile_frame.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=(0, 10))
        
        ttk.Label(mobile_frame, text="Teléfono Origen:").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.mobile_from_entry = ttk.Entry(mobile_frame, width=30)
        self.mobile_from_entry.grid(row=0, column=1, pady=2, padx=(5, 0))
        
        ttk.Label(mobile_frame, text="Teléfono Destino:").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.mobile_to_entry = ttk.Entry(mobile_frame, width=30)
        self.mobile_to_entry.grid(row=1, column=1, pady=2, padx=(5, 0))
        
        ttk.Label(mobile_frame, text="Monto:").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.mobile_amount_entry = ttk.Entry(mobile_frame, width=30)
        self.mobile_amount_entry.grid(row=2, column=1, pady=2, padx=(5, 0))
        
        ttk.Label(mobile_frame, text="Descripción:").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.mobile_desc_entry = ttk.Entry(mobile_frame, width=30)
        self.mobile_desc_entry.grid(row=3, column=1, pady=2, padx=(5, 0))
        
        ttk.Button(mobile_frame, text="Validar Teléfono", command=self.validate_phone).grid(row=4, column=0, pady=5)
        ttk.Button(mobile_frame, text="Transferencia Móvil", command=self.create_mobile_transfer).grid(row=4, column=1, pady=5)
        ...
        ttk.Button(utils_frame, text="Ver Contactos de Bancos", command=self.show_bank_contacts).grid(row=0, column=0, padx=5)
        ttk.Button(utils_frame, text="Health Check", command=self.health_check).grid(row=0, column=1, padx=5)
    ...
    def create_transaction_tab(self):
        ...
        ttk.Label(filter_frame, text="Número de Cuenta:").grid(row=0, column=0, sticky=tk.W, pady=2)
        ...
        ttk.Button(filter_frame, text="Filtrar por Cuenta", command=self.filter_transactions).grid(row=0, column=2, padx=(10, 0))
        ttk.Button(filter_frame, text="Ver Todas", command=self.load_transactions).grid(row=0, column=3, padx=(5, 0))
        
        # Lista de transacciones
        list_frame = ttk.LabelFrame(transaction_frame, text="Historial de Transacciones", padding="10")
        list_frame.grid(row=1, column=0, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        
        self.transaction_tree = ttk.Treeview(list_frame, columns=("ID", "De", "Para", "Monto", "Estado", "Tipo", "Fecha"), show="headings", height=15)
        ...
        # Botones
        ttk.Button(list_frame, text="Actualizar Lista", command=self.load_transactions).grid(row=1, column=0, pady=(10, 0))
    ...
    def create_auth_tab(self):
        ...
        login_frame.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 10), pady=(0, 10))
        
        ttk.Label(login_frame, text="Usuario:").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.login_username_entry = ttk.Entry(login_frame, width=30)
        self.login_username_entry.grid(row=0, column=1, pady=2, padx=(5, 0))
        
        ttk.Label(login_frame, text="Contraseña:").grid(row=1, column=0, sticky=tk.W, pady=2)
        ...
        ttk.Button(status_frame, text="Verificar Estado", command=self.check_auth_status).grid(row=1, column=0, pady=5)
        ttk.Button(status_frame, text="Cerrar Sesión", command=self.logout).grid(row=2, column=0, pady=5)
    ...
    def check_server_status(self):
        ...
        try:
            ...
            if response.status_code == 200:
                self.server_status_label.config(text="Estado: ✓ Servidor en línea", foreground="green")
                return True
            else:
                self.server_status_label.config(text="Estado: ✗ Servidor con problemas", foreground="orange")
                return False
        except:
            self.server_status_label.config(text="Estado: ✗ Servidor fuera de línea", foreground="red")
            return False
        ...
        if response and response.status_code == 200:
            ...
            for account in accounts:
                self.account_tree.insert("", "end", values=(
                    account["id"], account["number"], account["currency"], 
    ...
    def update_account_balance(self):
        ...
        new_balance = simpledialog.askfloat("Actualizar Saldo", 
                                          f"Saldo actual: {current_balance}\nNuevo saldo:", 
                                          initialvalue=float(current_balance))
        if new_balance is not None:
            data = {"balance": new_balance}
            response = self.api_request("PUT", f"/api/accounts/{account_id}/balance", data)
            if response and response.status_code == 200:
        ...
        if response and response.status_code == 200:
            ...
            for link in links:
                ...
                if link.get("created_at"):
                    try:
                        created_date = datetime.fromisoformat(link["created_at"].replace("Z", "+00:00")).strftime("%Y-%m-%d %H:%M")
                    except:
                        created_date = link["created_at"]
                        
                self.phone_tree.insert("", "end", values=(
                    link["id"], link["account_number"], link["phone"], created_date
                ))
        else:
            messagebox.showerror("Error", "No se pudo cargar la lista de enlaces telefónicos")
    ...
    def delete_phone_link(self):
        ...
        if messagebox.askyesno("Confirmar", f"¿Eliminar enlace del teléfono '{phone}'?"):
    ...
    def create_transfer(self):
        ...
        if not all(data[field] for field in required_fields):
            messagebox.showwarning("Datos Incompletos", "Los campos de origen, destino y monto son obligatorios")
            return
        ...
        try:
            ...
            messagebox.showwarning("Error", "Las cuentas deben ser IDs numéricos y el monto debe ser un número válido")
            return
    ...
    def show_bank_contacts(self):
        ...
        if response and response.status_code == 200:
            ...
            # Crear Treeview
            tree = ttk.Treeview(contacts_window, columns=("Código", "Nombre", "URL", "SSH"), show="headings")
            ...
            for contact in contacts:
                tree.insert("", "end", values=(
                    contact.get("code", ""), contact.get("name", ""), 
                    contact.get("url", ""), contact.get("ssh_host", "")
                ))
    ...
    def health_check(self):
        ...
        if response and response.status_code == 200:
            data = response.json()
            info = (f"Estado: {data.get('status', 'unknown')}\n"
        ...
        if response and response.status_code == 200:
            ...
            for transaction in transactions:
                ...
                if transaction.get("created_at"):
                    try:
                        created_date = datetime.fromisoformat(transaction["created_at"].replace("Z", "+00:00")).strftime("%Y-%m-%d %H:%M")
                    except:
    ...
    def filter_transactions(self):
        ...
        response = self.api_request("GET", f"/api/accounts/{account_number}/transactions")
        if response and response.status_code == 200:
            data = response.json()
            transactions = data.get("data", [])
    ...
    def login(self):
        ...
        if not all(data.values()):
            messagebox.showwarning("Datos Incompletos", "Usuario y contraseña son obligatorios")
            return
    ...
    def check_auth_status(self):
        ...
        if response and response.status_code == 200:
            ...
            if data.get("authenticated"):
                ...
                self.auth_status_label.config(text=f"Autenticado como: {username}", foreground="green")
            else:
                self.auth_status_label.config(text="No autenticado", foreground="red")
        else:
            self.auth_status_label.config(text="Error al verificar estado", foreground="orange")
