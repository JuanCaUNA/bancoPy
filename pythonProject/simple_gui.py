...
class SimpleBankingGUI:
    ...
    def setup_simple_ui(self):
        ...
        # Título
        title_label = tk.Label(main_frame, text="Sistema Bancario SINPE", 
                              font=("Arial", 16, "bold"))
        title_label.pack(pady=(0, 20))
        
        # Estado del servidor
        self.server_frame = tk.LabelFrame(main_frame, text="Control del Servidor", padx=10, pady=10)
        self.server_frame.pack(fill=tk.X, pady=(0, 20))
        
        self.status_label = tk.Label(self.server_frame, text="Estado: Verificando...", fg="blue")
        self.status_label.pack(side=tk.LEFT)
        
        self.start_btn = tk.Button(self.server_frame, text="Iniciar Servidor", 
                                  command=self.start_server)
        self.start_btn.pack(side=tk.RIGHT, padx=(5, 0))
        
        self.check_btn = tk.Button(self.server_frame, text="Verificar", 
    ...
    def create_users_tab(self):
        ...
        tk.Label(form_frame, text="Teléfono:").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.user_phone = tk.Entry(form_frame, width=30)
        self.user_phone.grid(row=2, column=1, pady=2, padx=(5, 0))
        
        tk.Label(form_frame, text="Contraseña:").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.user_password = tk.Entry(form_frame, width=30, show="*")
        self.user_password.grid(row=3, column=1, pady=2, padx=(5, 0))
        
        tk.Button(form_frame, text="Crear Usuario", 
                 command=self.create_user).grid(row=4, column=0, columnspan=2, pady=10)
        ...
        tk.Button(list_frame, text="Actualizar Lista", 
    ...
    def create_accounts_tab(self):
        ...
        # Formulario
        form_frame = tk.LabelFrame(accounts_frame, text="Crear Cuenta", padx=10, pady=10)
        form_frame.pack(fill=tk.X, padx=10, pady=10)
        
        tk.Label(form_frame, text="Saldo Inicial:").grid(row=0, column=0, sticky=tk.W, pady=2)
        ...
        tk.Label(form_frame, text="User ID (opcional):").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.account_user_id = tk.Entry(form_frame, width=30)
        self.account_user_id.grid(row=1, column=1, pady=2, padx=(5, 0))
        
        tk.Button(form_frame, text="Crear Cuenta", 
                 command=self.create_account).grid(row=2, column=0, columnspan=2, pady=10)
        ...
        tk.Button(list_frame, text="Actualizar Lista", 
                 command=self.load_accounts).pack(pady=5)
    ...
    def create_transactions_tab(self):
        ...
        tk.Label(form_frame, text="Cuenta Origen (ID):").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.transfer_from = tk.Entry(form_frame, width=30)
        self.transfer_from.grid(row=0, column=1, pady=2, padx=(5, 0))
        
        tk.Label(form_frame, text="Cuenta Destino (ID):").grid(row=1, column=0, sticky=tk.W, pady=2)
        ...
        tk.Button(form_frame, text="Realizar Transferencia", 
                 command=self.create_transfer).grid(row=3, column=0, columnspan=2, pady=10)
        ...
        tk.Button(list_frame, text="Actualizar Lista", 
                 command=self.load_transactions).pack(pady=5)
    ...
    def start_server(self):
        ...
        def run_server():
            try:
                ...
                self.root.after(0, lambda: messagebox.showerror("Error", f"Error al iniciar servidor: {e}"))
    ...
    def api_call(self, endpoint, method="GET", data=None):
        ...
        try:
            if REQUESTS_AVAILABLE:
                ...
                messagebox.showwarning("Error", "requests no está disponible. Instale con: pip install requests")
                return None
    ...
    def create_account(self):
        """Crear cuenta"""
        data = {
            "balance": self.account_balance.get(),
            "currency": "CRC"
        }
    ...
    def create_transfer(self):
        ...
        try:
            ...
            messagebox.showwarning("Error", "Verifique que los IDs sean números y el monto sea válido")
            return
    ...
    def load_transactions(self):
        ...
        if result and result.get("data"):
            ...
            for trans in transactions:
                ...
                text += f"De: {trans['from_account_id']} → Para: {trans['to_account_id']}\n"
            ...
            self.transactions_text.delete(1.0, tk.END)
            self.transactions_text.insert(1.0, "No se pudieron cargar las transacciones")
