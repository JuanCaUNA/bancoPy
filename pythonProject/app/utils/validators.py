...
def validate_sinpe_payload(data: Dict[Any, Any]) -> Tuple[bool, str]:
    ...
    if 'account_number' not in sender or 'bank_code' not in sender or 'name' not in sender:
        return False, "Sender debe tener account_number, bank_code y name"
    
    # Validar receiver  
    receiver = data.get('receiver', {})
    if 'account_number' not in receiver or 'bank_code' not in receiver or 'name' not in receiver:
...
def validate_sinpe_movil_payload(data: Dict[Any, Any]) -> Tuple[bool, str]:
    ...
    if not validate_phone_format(receiver['phone_number']):
        return False, f"Número de teléfono receiver inválido: {receiver['phone_number']}"
